...
export const Topbar = ({ onMenuClick }: TopbarProps) => {
  ...
  const { notifications, unreadCount, hasNew, clearNew } = useNotifications(
    userDoc?.id,
  );
  ...
  const currentLabel =
    Object.entries(breadcrumbLabels).find(([path]) => {
  ...
  return (
    <header className="h-14 flex items-center gap-2 sm:gap-4 px-3 sm:px-6 border-b border-border bg-background/80 backdrop-blur-sm shrink-0 z-50 relative shadow-sm">
      {/* Mobile hamburger */}
      <button
        onClick={onMenuClick}
        ...
        <AnimatePresence>
          {notifOpen && (
            <motion.div
              ...
              className={cn(
                "absolute top-10 w-72 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden",
                  ...
                  <div className="py-1">
                    {notifications.slice(0, 8).map((n) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (n.taskId && n.spaceId) {
                              navigate(
                                `/spaces/${n.spaceId}/tasks/${n.taskId}`,
                              );
                              setNotifOpen(false);
                            }
                          }}
                        >
                          <Icon
                            className={cn("w-4 h-4 shrink-0 mt-0.5", color)}
                          />
                  ...
                  {searchResults.length > 0 ? (
                    searchResults.map((r) => (
                      <button
                        ...
                        onClick={() => {
                          if (r.type === "task")
                            navigate(`/spaces/${r.spaceId}/tasks/${r.id}`);
