...
function TableView({
  ...
  const [sortField, setSortField] = useState<
    "title" | "status" | "priority" | "deadline" | "progress"
  >("deadline");
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50">
            <tr className="text-xs font-semibold text-muted-foreground border-b border-border">
              ...
              <th className="text-start px-3 py-2.5 hidden sm:table-cell">
                <SortBtn field="status" label="Status" />
              </th>
              <th className="text-start px-3 py-2.5 hidden md:table-cell">
                <SortBtn field="priority" label="Priority" />
              </th>
              <th className="text-start px-3 py-2.5 hidden md:table-cell">
                <SortBtn field="deadline" label="Deadline" />
              </th>
              <th className="text-start px-3 py-2.5 hidden lg:table-cell">
                Assignees
              </th>
              <th className="text-start px-3 py-2.5 hidden lg:table-cell">
                <SortBtn field="progress" label="%" />
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {sorted.map((task) => {
              ...
              return (
                <tr
                  key={task.id}
                  onClick={() =>
                    onNavigate(`/spaces/${spaceId}/tasks/${task.id}`)
                  }
                  ...
                  <td className="px-3 py-2.5 hidden lg:table-cell">
                    <div className="flex -space-x-1.5">
                      {assignees.slice(0, 3).map((m) => (
                        <div
                          key={m.id}
                          className="w-5 h-5 rounded-full bg-primary/30 border border-card flex items-center justify-center text-[9px] font-bold text-primary"
                        >
