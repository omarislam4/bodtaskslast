...
function GanttView({
  ...
  if (tasksWithDates.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
        ...
        <p className="font-semibold">No tasks with deadlines</p>
      </div>
    );
  }

  const minDate = new Date(tasksWithDates[0].startDate || tasksWithDates[0].createdAt);
  ...
  const STATUS_COLORS: Record<string, string> = {
    todo: "#6b7280", "in-progress": "#3b82f6", review: "#f59e0b", done: "#10b981", blocked: "#ef4444",
  };
            ...
            >
              ...
              <div className="flex-1 h-7 relative bg-muted/50 rounded-lg overflow-hidden">
                <div
                  ...
                  style={{ left: `${leftPct}%`, width: `${widthPct}%`, backgroundColor: color + "30", borderLeft: `3px solid ${color}` }}
                >
              ...
              <div className="w-16 text-xs text-muted-foreground shrink-0">{format(new Date(task.deadline!), "MMM d")}</div>
            </div>
          );
        })}
      </div>
