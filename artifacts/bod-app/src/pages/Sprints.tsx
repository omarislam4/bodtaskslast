...
import { Zap, Plus, X, CheckCircle2, Clock, ChevronRight, Archive, Play } from "lucide-react";
        ...
        <div className="space-y-6">
          ...
          {activeSprints.length > 0 && (
            <div>
              ...
              {activeSprints.map(sprint => <SprintCard key={sprint.id} sprint={sprint} tasks={getSprintTasks(sprint)} doneCount={getDoneCount(sprint)} spaces={spaces} onRemove={removeFromSprint} onStatus={changeStat} onNavigate={navigate} />)}
            </div>
          )}
          ...
          {planningSprints.length > 0 && (
            <div>
              ...
              {planningSprints.map(sprint => <SprintCard key={sprint.id} sprint={sprint} tasks={getSprintTasks(sprint)} doneCount={getDoneCount(sprint)} spaces={spaces} onRemove={removeFromSprint} onStatus={changeStat} onNavigate={navigate} />)}
            </div>
          )}
          ...
          {completedSprints.length > 0 && (
            <details className="group">
              ...
              {completedSprints.map(sprint => <SprintCard key={sprint.id} sprint={sprint} tasks={getSprintTasks(sprint)} doneCount={getDoneCount(sprint)} spaces={spaces} onRemove={removeFromSprint} onStatus={changeStat} onNavigate={navigate} />)}
            </details>
          )}
...
function SprintCard({ sprint, tasks, doneCount, spaces, onRemove, onStatus, onNavigate }: {
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {sprint.status === "planning" && (
            <button onClick={() => onStatus(sprint, "active")} className="flex items-center gap-1 text-xs px-3 py-1.5 bg-emerald-500/10 text-emerald-500 rounded-lg hover:bg-emerald-500/20 transition-all">
              <Play className="w-3 h-3" /> Start
            </button>
          )}
          {sprint.status === "active" && (
            <button onClick={() => onStatus(sprint, "completed")} className="flex items-center gap-1 text-xs px-3 py-1.5 bg-primary/10 text-primary rounded-lg hover:bg-primary/20 transition-all">
